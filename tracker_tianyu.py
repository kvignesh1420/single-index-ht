...
class Tracker:
    def __init__(self, context):
        ...
        self.iteration_gradient_spectral_norm=OrderedDict()
        self.iteration_gradient_frobenuis_norm=OrderedDict()
        self.iteration_gradient_stable_rank=OrderedDict()
        self.iteration_gradient_weight_esd = OrderedDict()
        self.iteration_gradient_weight_vals = OrderedDict()
    ...
    def probe_iteration_gradient_weights(self, teacher, student, iteration):

           
        for idx, layer in enumerate(student.layers):
            if idx==0:
                W = layer.weight.grad.detach().clone()
                name="{}G{}_iteration{}".format(self.context["vis_dir"], idx, iteration)
                self.plot_tensor(M=W, name=name)
                if iteration not in self.iteration_gradient_weight_vals:
                    self.iteration_gradient_weight_vals[iteration] = OrderedDict()
                self.iteration_gradient_weight_vals[iteration][idx] = W
                S_W = torch.linalg.svdvals(W)
                self.plot_svd(S_M=S_W, name=name)
                WtW = W.t() @ W
                S_WtW = torch.linalg.svdvals(WtW)
                ...
                #self.plot_esd(S_MtM=nz_S_WtW , name=name)
                self.plot_weight_esd(S_MtM=nz_S_WtW, name=name,spectral_norm=spectral_norm,fnorm=fnorm,stable_rank=stable_rank)
                if iteration not in self.iteration_gradient_weight_esd:
                    self.iteration_gradient_weight_esd[iteration] = OrderedDict()
                self.iteration_gradient_weight_esd[iteration][idx] = nz_S_WtW
                if iteration not in self.iteration_gradient_spectral_norm:
                    self.iteration_gradient_spectral_norm[iteration] = OrderedDict()
                self.iteration_gradient_spectral_norm[iteration][idx] = spectral_norm
                if iteration not in self.iteration_gradient_frobenuis_norm:
                    self.iteration_gradient_frobenuis_norm[iteration] = OrderedDict()
                self.iteration_gradient_frobenuis_norm[iteration][idx] =fnorm
                if iteration not in self.iteration_gradient_stable_rank:
                    self.iteration_gradient_stable_rank[iteration] = OrderedDict()
                self.iteration_gradient_stable_rank[iteration][idx] = stable_rank
            else:
                pass
    ...
    def plot_initial_final_gradient_weight_vals(self):
        epochs = list(self.iteration_gradient_weight_vals.keys())
        initial_epoch = epochs[0]
        final_epoch = epochs[-1]
        for idx in self.iteration_gradient_weight_vals[0]:
            if idx==0:
                initial_W = self.iteration_gradient_weight_vals[initial_epoch][idx]
                final_W = self.iteration_gradient_weight_vals[final_epoch][idx]
                initial_vals = torch.flatten(initial_W).cpu().numpy()
                final_vals = torch.flatten(final_W).cpu().numpy()
                plt.hist(initial_vals, bins=100, density=True, color="orange", alpha=0.5, edgecolor='red', label="initial")
                plt.hist(final_vals, bins=100, density=True, color="violet", alpha=0.5, edgecolor='blue', label="epoch{}".format(final_epoch))
                ...
                plt.ylabel("density(vals)")
                plt.legend()
                name="{}G{}".format(self.context["vis_dir"], idx)
                plt.savefig("{}_initial_final_vals.jpg".format(name))
                plt.clf()
            else:
                pass
    ...
    def plot_initial_final_gradient_weight_esd(self):
        epochs = list(self.iteration_gradient_weight_esd.keys())
        initial_epoch = epochs[0]
        final_epoch = epochs[-1]
        for idx in self.epoch_weight_esd[0]:
            if idx==0:
                initial_S_WtW = self.iteration_gradient_weight_esd[initial_epoch][idx]
                final_S_WtW = self.iteration_gradient_weight_esd[final_epoch][idx]
                initial_vals = np.log10(initial_S_WtW.cpu().numpy())
                final_vals = np.log10(final_S_WtW.cpu().numpy())
                plt.hist(initial_vals, bins=100, log=True, density=True, color="red", alpha=0.5, edgecolor='red', label="initial")
                plt.hist(final_vals, bins=100, log=True, density=True, color="blue", alpha=0.5, edgecolor='blue', label="epoch{}".format(final_epoch))
